defmodule RyuiWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: RyuiWeb.Gettext
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(RyuiWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(RyuiWeb.Gettext, "errors", msg, opts)
    end
  end
  ...
  def section(assigns) do
    ...
    <div>
      <div class="mb-8">
        <h1 id={@id} class="text-2xl font-semibold underline">
          {@title}
        </h1>
        <div :if={@inner_block} class="mt-2 text-base-content/80 text-sm italic">
          {render_slot(@inner_block)}
        </div>
      </div>
      <%= for {example, i} <- Enum.with_index(@example) do %>
        ...
        <div class="tabs tabs-lift mb-4 last:mb-0">
          ...
          <label class="tab">
            ...
            <.icon name="hero-code-bracket" /><span class="ml-1">Usage</span>
          </label>
